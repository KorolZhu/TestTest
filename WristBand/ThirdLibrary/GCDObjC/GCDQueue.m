...
- (instancetype)initSerialWithName:(NSString *)name {
    return [self initWithDispatchQueue:dispatch_queue_create(name.UTF8String, DISPATCH_QUEUE_SERIAL)];
}
...
- (instancetype)initConcurrentWithName:(NSString *)name {
    return [self initWithDispatchQueue:dispatch_queue_create(name.UTF8String, DISPATCH_QUEUE_CONCURRENT)];
}
